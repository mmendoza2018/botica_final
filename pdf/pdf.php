<?php
	...
	session_start();
	// $select_clientes= "SELECT * FROM cliente WHERE DNI_cli={$_GET['dni']}";
  //   $res = mysqli_query($mysqli,$select_clientes);
  //   $respuesta = mysqli_fetch_row($res);

		$select_clientes= "SELECT cliente.*, ventas.* FROM cliente INNER JOIN ventas on ventas.dni_c=cliente.DNI_cli WHERE ventas.id_venta={$_GET['boleta']}";
		$res = mysqli_query($mysqli,$select_clientes);
		$respuesta = mysqli_fetch_row($res);
	...
	<table class="margin-top-30">
		<tbody>
			<tr>
				...
				<td>
					NUMERO DE TELEFONO: <span class="bold">964 909 085</span>
				</td>
				<td>
					NRO BOLETA: <span class="bold"><?php echo $_GET['boleta']; ?></span>
				</td>
			</tr>
			<tr>
				<td colspan="3">
					DIRECCION: <span class="bold">Av. General Varela 763 -B la Tomilla Cayma</span>
				</td>
			</tr>
		</tbody>
	</table>
	...
	<table class="margin-top table">
		<tbody>
			<tr>
				...
				<td>
					FECHA: <span class="bold"><?php echo date('Y/m/d H:i:s'); ?></span>
				</td>
			</tr>
			<tr>
				<td colspan="2">
					NOMBRE VENDEDOR: <span class="bold"><?php echo $_SESSION["nom_usuario"];?></span>
				</td>
			</tr>
		</tbody>
	</table>
	...
	<table class="margin-top table">
		...
		<tbody >
			<?php
				...
				WHERE det_venta.id_v01=".$_GET['boleta'];
